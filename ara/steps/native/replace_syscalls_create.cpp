...
namespace ara::step {
	...
	namespace {
		...
		PyObject* handle_tcb_ref_param(IRBuilder<>& Builder, Value* tcb_ref, Value* the_tcb, Logger& logger) {
		...
		PyObject* replace_call_with_true(CallBase* call) {
			// return true;
		...
		BasicBlock* create_bb(graph::Graph& graph, object task) {
	...
	PyObject* ReplaceSyscallsCreate::replace_task_create_static(graph::Graph& graph, object task) {
		Module& module = graph.get_module();
		BasicBlock* bb = create_bb(graph, task);
	...
	PyObject* ReplaceSyscallsCreate::replace_task_create_initialized(graph::Graph& graph, object task) {
		Module& module = graph.get_module();
		BasicBlock* bb = create_bb(graph, task);
		std::string tcb_name = extract<std::string>(task.attr("impl").attr("tcb").attr("name"));
		CallBase* old_create_call = dyn_cast<CallBase>(&bb->front());
	...
	PyObject* ReplaceSyscallsCreate::replace_task_create(graph::Graph& graph, PyObject* pyo_task) {
		...
		if (init_type == "static") {
			return replace_task_create_static(graph, task);
		} else if (init_type == "initialized") {
			return replace_task_create_initialized(graph, task);
		} else {
