...
namespace ara::step {
	...
	namespace {
		...
		class CallgraphImpl {
		  ...
		  public:
            // TODO
			CallgraphImpl(CFGraph& cfg_obj, graph::CFG& cfg, CaGraph& callg_obj, graph::Callgraph& callgraph, SVF::PTACallGraph& svf_callgraph, llvm::Module& mod, Logger& logger)
			    : cfg_obj(cfg_obj), cfg(cfg), callg_obj(callg_obj), callgraph(callgraph), svf_callgraph(svf_callgraph), mod(mod), logger(logger) {
				for (auto function : boost::make_iterator_range(
				         boost::vertices(cfg.get_functions(cfg_obj)))) {
		...
		void map_callgraph(CFGraph& cfg_obj, graph::CFG& cfg, graph::Callgraph& callgraph, SVF::PTACallGraph& svf_callgraph, llvm::Module& mod, Logger& logger) {
			graph_tool::gt_dispatch<>()([&](auto& g) { CallgraphImpl(cfg_obj, cfg, g, callgraph, svf_callgraph, mod, logger); },
	...
	void Callgraph::run() {
		...
		if (*dump.get()) {
			...
			llvm::json::Value printer_conf(llvm::json::Object{
			    {"name", "Printer"}, {"dot", dot_file + ".dot"}, {"graph_name", "Callgraph"}, {"subgraph", "callgraph"}});
