...
namespace ara::step {
	...
	/*
	    ...
	    If you provide the error_with_obj obj then will this function print the python Type of this object. (Only if
	   cond == false)
	*/
	...
	void RemoveSyscallBody::run() {
		...
		py_assert(PyFunction_Check(get_syscalls), "ara.os.get_posix_syscalls() is not a python function!",
		          get_syscalls);
		...
		py_assert(syscall_iter != nullptr,
		          "ara.os.get_posix_syscalls() python function returned an object that cannot be cast to an Iterator!",
		          syscall_list);
		...
		while ((syscall = PyIter_Next(syscall_iter))) {
			...
			// else {
			//    logger.err() << "Could not find function with name \'" << syscall_name << "\' in LLVM IR." <<
			//    std::endl;
			//}
