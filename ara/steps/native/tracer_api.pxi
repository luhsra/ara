...
cdef public void py_tracer_entity_on_node(object tracer, object entity, object node, int graph_type) except *:
    g_node = GraphNode(node, graph_type)
    tracer.entity_on_node(entity, g_node)

cdef public void py_tracer_entity_is_looking_at(object tracer, object entity, object path, int graph_type) except *:
...
cdef public void py_tracer_go_to_node(object tracer, object entity, object path, int graph_type, bint forward) except *:
    assert path is not None, "path is not containing any edges"
    g_path = GraphPath(path, graph_type)
...
cdef public object py_tracer_get_vertex_by_id(object tracer, unsigned long v_id, int graph_type):
    return tracer.get_vertex_by_id(v_id, graph_type)

cdef public object py_tracer_add_edge_to_path(object tracer, object edge_list, unsigned long source, unsigned long target, int graph_type):
