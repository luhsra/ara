...
namespace ara::step {
	...
	std::string PosixClangGlobalTest::get_description() { return "Checks if Clang/LLVM is working as expected for static instance detection in POSIX model"; }
	...
	void PosixClangGlobalTest::run() {
        ...
        if (!static_case && file != "appl/POSIX/objs/clang_global_var_dynamic.ll") {
            this->fail("unknown input file \"" + file + "\" (arbitrary input files are not allowed for this test case)");
        }
        ...
        if (init_type == nullptr || init_type->getStructName().str().length() == 0) {
            ...
            if (static_case) {
                this->fail("static global have a typed initializer! Strange behaviour of Clang/LLVM is gone. We can now no more rely on it.");
            }
        }
	}
