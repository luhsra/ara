...
cdef class Vertex:
    ...
    def __cinit__(self, PyGraph graph, string name):
        # prevent double constructions
        # https://github.com/cython/cython/wiki/WrappingSetOfCppClasses
        if type(self) != Vertex:
            return
        self._c_vertex = make_shared[cgraph.Vertex](&graph._c_graph, name)
...
cdef class Alarm(Vertex):
    ...
    def __cinit__(self, PyGraph graph, string name):
        self._c_alarm = make_shared[cgraph.Alarm](&graph._c_graph, name)
...
cdef class Counter(Vertex):
    ...
    def __cinit__(self, PyGraph graph, string name):
        self._c_counter = make_shared[cgraph.Counter](&graph._c_graph, name)
...
cdef class Event(Vertex):
    ...
    def __cinit__(self, PyGraph graph, string name):
        self._c_event = make_shared[cgraph.Event](&graph._c_graph, name)
...
cdef class ISR(Vertex):
    ...
    def __cinit__(self, PyGraph graph, string name):
        self._c_isr = make_shared[cgraph.ISR](&graph._c_graph, name)
...
cdef class Resource(Vertex):
    ...
    def __cinit__(self, PyGraph graph, string name):
        self._c_resource = make_shared[cgraph.Resource](&graph._c_graph, name)
...
cdef class Task(Vertex):
    ...
    def __cinit__(self, PyGraph graph, string name):
        self._c_task = make_shared[cgraph.Task](&graph._c_graph, name)
