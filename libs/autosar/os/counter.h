...
namespace os {
	...
	class UnencodedCounter : public Counter {
	  public:
		...
		constexpr UnencodedCounter(TickType mav, TickType tpb, TickType mc)
		    : value(0), maxallowedvalue(mav), ticksperbase(tpb), mincycle(mc) {}

		forceinline Ticks getValue() const { return value; }
	...
	class EncodedCounter : public Counter {
	  public:
		...
		constexpr EncodedCounter(TickType mav, TickType tpb, TickType mc)
		    : value(0), maxallowedvalue(mav), ticksperbase(tpb), mincycle(mc) {}
