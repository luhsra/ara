	...
	namespace redundant {
		...
		class Plain {
		  ...
		  public:
			...
			inline void set(T x) const { data = x; }
		...
		class HighParity {
		  ...
		  public:
			...
			inline T get() const { return (T)((uint32_t)data & 0x7FFFFFFF); }
		...
		class MergedDMR {
		  ...
		  public:
			...
			inline void set(uint16_t x) const { data = (x) | (x << 16); }

			inline uint16_t get() const { return data & 0xffff; }

			inline bool check() const {
				if ((data & 0xffff) != (data >> 16)) {
					CALL_HOOK(FaultDetectedHook, DMRdetected, data & 0xffff, data >> 16);
					return false;
				}
				return true;
			}
		...
		class DMRWithLinkage {
		  ...
		  public:
			...
			inline T get() const { return data0; }

			inline bool check() const {
				if (data0 != data1) {
					CALL_HOOK(FaultDetectedHook, DMRdetected, (uint32_t)data0, (uint32_t)data1);
					return false;
				}
				return true;
			}
