...
#define ISR2(taskname)                                                                                                 \
	...
	EXTERN_C_DECL void OSEKOS_ISR_##taskname(const arch::IRQ::Context&)

#else

#define ISR2(taskname)                                                                                                 \
	__attribute__((always_inline)) __attribute__((used)) EXTERN_C_DECL void OSEKOS_ISR_##taskname(void)
 ...
 * @satisfies{13,2,5}
 */
#define TASK(taskname) noinline EXTERN_C_DECL void OSEKOS_TASK_FUNC_##taskname(void)

#define ActivateTask(x) OSEKOS_ActivateTask(OSEKOS_TASK_##x)

#define ChainTask(x) OSEKOS_ChainTask(OSEKOS_TASK_##x)

#define TerminateTask() OSEKOS_TerminateTask()

#define Schedule() OSEKOS_Schedule()
...
#define GetResource(x) OSEKOS_GetResource(OSEKOS_RESOURCE_##x)

#define GetTaskID(x) OSEKOS_GetTaskID(x)

#define ReleaseResource(x) OSEKOS_ReleaseResource(OSEKOS_RESOURCE_##x)

#define DeclareEvent(x)                                                                                                \
	extern const EventMaskType OSEKOS_EVENT_##x;                                                                       \
	static dosek_unused const EventMaskType& x = OSEKOS_EVENT_##x

#define SetEvent(task, event) OSEKOS_SetEvent(OSEKOS_TASK_##task, event)

#define GetEvent(task, event) OSEKOS_GetEvent(OSEKOS_TASK_##task, event)

#define ClearEvent(event) OSEKOS_ClearEvent(event)

#define WaitEvent(event) OSEKOS_WaitEvent(event)

#define DeclareAlarm(x)                                                                                                \
	extern const AlarmType OSEKOS_ALARM_##x;                                                                           \
	static dosek_unused const AlarmType& x = OSEKOS_ALARM_##x;

#define ALARMCALLBACK(x) EXTERN_C_DECL void OSEKOS_ALARMCB_##x()

#define GetAlarm(x, tick) OSEKOS_GetAlarm(OSEKOS_ALARM_##x, tick)

#define SetRelAlarm(x, inc, period) OSEKOS_SetRelAlarm(OSEKOS_ALARM_##x, inc, period)

#define SetAbsAlarm(x, inc, period) OSEKOS_SetRelAlarm(OSEKOS_ALARM_##x, inc, period)

#define CancelAlarm(x) OSEKOS_CancelAlarm(OSEKOS_ALARM_##x)

#define GetAlarmBase(x, b) OSEKOS_GetAlarmBase(OSEKOS_ALARM_##x, b)
...
#define AdvanceCounter(x) OSEKOS_AdvanceCounter(OSEKOS_COUNTER_##x)

#define IncrementCounter(x) OSEKOS_AdvanceCounter(OSEKOS_COUNTER_##x)

#define GetCounter(x) OSEKOS_GetCounter(OSEKOS_COUNTER_##x)

#define DeclareDevice(x)                                                                                               \
	extern const DeviceType OSEKOS_DEVICE_##x;                                                                         \
	static dosek_unused const DeviceType& x = OSEKOS_DEVICE_##x

#define ActivateDevice(x) OSEKOS_ActivateDevice(OSEKOS_DEVICE_##x)

#define DeactivateDevice(x) OSEKOS_DeactivateDevice(OSEKOS_DEVICE_##x)

#define StartOS(x) OSEKOS_StartOS(x)
...
#define ShutdownOS(STATUS) OSEKOS_ShutdownOS(STATUS)

#define DisableAllInterrupts() OSEKOS_DisableAllInterrupts()
#define EnableAllInterrupts() OSEKOS_EnableAllInterrupts()
#define SuspendAllInterrupts() OSEKOS_SuspendAllInterrupts()
#define ResumeAllInterrupts() OSEKOS_ResumeAllInterrupts()
#define SuspendOSInterrupts() OSEKOS_SuspendOSInterrupts()
#define ResumeOSInterrupts() OSEKOS_ResumeOSInterrupts()
...
extern StatusType OSEKOS_ActivateDevice(DeviceType r);
extern StatusType OSEKOS_DeactivateDevice(DeviceType r);
