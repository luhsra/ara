...
/**
 * @brief gets all of the specific argument of all possible varations, E.g if the call argument is also the function
 * argument, there could be many possible values The function selects the right one, by comparing the argument
 * instruction references with the call references of the current abstraction instance
...
/**
 * @brief gets the specific argument of all possible varations, E.g if the call argument is also the function argument,
 * there could be many possible values The function selects the right one, by comparing the argument instruction
 * references with the call references of the current abstraction instance
...
void get_call_relative_argument(std::any& any_value, llvm::Value*& llvm_value, argument_data argument,
	...
	if (argument.multiple == false) {
		...
		for (auto tmp : *call_references) {
			if (tmp != nullptr)
				stream << "current: " << print_argument(tmp) << std::endl;
			else
		...
		for (auto argument_calles : argument.argument_calles_list) {
			...
			if (tmp_argument_calles.empty())
				valid_candidates.emplace_back(
				    std::make_tuple(argument.any_list.at(index), argument.value_list.at(index), missmatch_list));
			++index;
		}
					...
					while (flag) {
						...
						for (auto candidate : valid_candidates) {
							...
							if (missmatch_list.empty() || missmatch_list.size() < index) {
								...
								// next possible candidate with equal min missmatch value detected, so store the
								// candidate
								candidate_indexes.emplace_back(candidate_index);
							}
							++candidate_index;
						...
						if (valid_candidates.size() == 1) {
							...
							llvm_value = std::get<llvm::Value*>(valid_candidates.front());
							return;
						} else if (valid_candidates.size() == 0)
							flag = false;
						// error detected
						else if (index > 100)
							flag = false;
 ...
 */
graph::shared_vertex create_task(graph::Graph& graph, OS::shared_abb abb, bool before_scheduler_start,
                                 std::vector<llvm::Instruction*>* call_references) {
	...
	if (error)
		...
		task->set_multiple_create(true);

	if (initial && !error) {
		std::hash<std::string> hash_fn;
		graph::shared_vertex function_vertex =
		    graph.get_vertex(hash_fn(function_reference_name + typeid(OS::Function).name()));
 ...
 * @return a shared pointer of the created abstraction instance
 */
graph::shared_vertex create_semaphore(graph::Graph& graph, OS::shared_abb abb, semaphore_type type,
                                      bool before_scheduler_start, std::vector<llvm::Instruction*>* call_references) {
	...
	if (success) {
		...
		if (error)
			...
			semaphore->set_multiple_create(true);
 ...
 * @return a shared pointer of the created abstraction instance
 */
graph::shared_vertex create_mutex(graph::Graph& graph, OS::shared_abb abb, resource_type type,
                                  bool before_scheduler_start, std::vector<llvm::Instruction*>* call_references) {
	...
	if (success) {
		...
		if (error)
			...
			resource->set_multiple_create(true);
 ...
 * @return a shared pointer of the created abstraction instance
 */
graph::shared_vertex create_queue(graph::Graph& graph, OS::shared_abb abb, bool before_scheduler_start,
                                  std::vector<llvm::Instruction*>* call_references) {
	...
	if (type != binary_semaphore) {
		...
		if (error)
			...
			queue->set_multiple_create(true);
 ...
 * @return a shared pointer of the created abstraction instance
 */
graph::shared_vertex create_event_group(graph::Graph& graph, OS::shared_abb abb, bool before_scheduler_start,
                                        std::vector<llvm::Instruction*>* call_references) {
	...
	if (error)
		...
		event_group->set_multiple_create(true);
 ...
 * @return a shared pointer of the created abstraction instance
 */
graph::shared_vertex create_queue_set(graph::Graph& graph, OS::shared_abb abb, bool before_scheduler_start,
                                      std::vector<llvm::Instruction*>* call_references) {
	...
	if (error)
		...
		queue_set->set_multiple_create(true);
 ...
 * @return a shared pointer of the created abstraction instance
 */
graph::shared_vertex create_timer(graph::Graph& graph, OS::shared_abb abb, bool before_scheduler_start,
                                  std::vector<llvm::Instruction*>* call_references) {
	...
	if (timer_autoreload == 0)
		timer->set_timer_type(oneshot);
	else
		timer->set_timer_type(autoreload);
	...
	if (error)
		...
		timer->set_multiple_create(true);

	if (initial && !error) {

		std::hash<std::string> hash_fn;
		graph::shared_vertex function_vertex =
		    graph.get_vertex(hash_fn(timer_definition_function + typeid(OS::Function).name()));
		auto function_reference = std::dynamic_pointer_cast<OS::Function>(function_vertex);
 ...
 * @return a shared pointer of the created abstraction instance
 */
graph::shared_vertex create_buffer(graph::Graph& graph, OS::shared_abb abb, bool before_scheduler_start,
                                   std::vector<llvm::Instruction*>* call_references) {
	...
	if (error)
		...
		buffer->set_multiple_create(true);
 ...
 * @return a shared pointer of the created abstraction instance
 */
graph::shared_vertex create_coroutine(graph::Graph& graph, OS::shared_abb abb, bool before_scheduler_start,
                                      std::vector<llvm::Instruction*>* call_references) {
	...
	if (error)
		...
		coroutine->set_multiple_create(true);
...
void detect_isrs(graph::Graph& graph) {
	...
	for (auto& vertex : vertex_list) {
		...
		if (abb->get_syscall_name().find("FromISR") != std::string::npos) {
			...
			if (!success)
				std::cerr << "isr could not created, because of recursion" << std::endl;
		}
	}
}
 ...
 */
bool create_abstraction_instance(graph::Graph& graph, graph::shared_vertex start_vertex, OS::shared_abb abb,
                                 bool before_scheduler_start, std::vector<llvm::Instruction*>* already_visited_calls,
                                 bool multiple_create, std::vector<shared_warning>* warning_list) {
	try {
		...
		if (list_contains_element(abb->get_call_target_instances(), typeid(OS::Mutex).hash_code())) {
			...
			resource_type type;
			std::string syscall_name = abb->get_syscall_name();
			if (syscall_name == "xQueueCreateMutex")
				type = binary_mutex;
			else if (syscall_name == "xSemaphoreCreateRecursiveMutex")
		...
		if (created_vertex) {
			...
			auto edge =
			    std::make_shared<graph::Edge>(&graph, abb->get_syscall_name(), start_vertex, created_vertex, abb);
 ...
 * @param warning_list list to store warning
 */
void iterate_called_functions(graph::Graph& graph, graph::shared_vertex start_vertex, OS::shared_function function,
                              llvm::Instruction* call_reference, std::vector<llvm::Instruction*> already_visited_calls,
	...
	for (auto& abb : abb_list) {
		...
		if (abb->get_call_type() == sys_call) {
			...
			if (abb->get_loop_information()) {
				...
				// std::cerr << "abb " << abb->get_name() << " with syscall "<< abb->get_syscall_name() << "in loop" <<
				// std::endl;
			}

			bool before_scheduler_start = false;

			if (abb->get_start_scheduler_relation() == before)
				before_scheduler_start = true;

			// check if abb syscall is creation syscall
			if (abb->get_syscall_type() == create) {

				if (!create_abstraction_instance(graph, start_vertex, abb, before_scheduler_start, calltree_references,
				                                 multiple_create, warning_list)) {
			...
			iterate_called_functions(graph, start_vertex, abb->get_called_function(),
			                         abb->get_call_instruction_reference(), already_visited_calls, calltree_references,
			                         warning_list);
		}
	}
}
...
namespace step {
	...
	/**
	 * @brief the run method of the FreeRTOSInstancesStep pass. This pass detects all FreeRTOS instances  and gets their
	 * characteristics
	 * @param graph project data structure
	 */
	...
	void FreeRTOSInstancesStep::run(graph::Graph& graph) {
		...
		do {
			...
			for (auto& vertex : vertex_list) {
				...
				std::vector<llvm::Instruction*> already_visited_calls;
				std::vector<llvm::Instruction*> calltree_references;
				iterate_called_functions(graph, coroutine, coroutine_definition, nullptr, already_visited_calls,
				                         &calltree_references, warning_list);
			}
	...
	std::vector<std::string> FreeRTOSInstancesStep::get_dependencies() { return {"IntermediateAnalysisStep"}; }
} // namespace step
