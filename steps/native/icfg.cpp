...
namespace ara::step {
	...
	namespace {
		...
		void check_icf(Graph& g, CFG& cfg, llvm::Module& mod, Logger& logger) {
			...
			boost::filtered_graph<Graph, boost::keep_all, ABBTypeFilter> non_calls(g, boost::keep_all(),
			                                                                       non_call_filter);
			...
			for (auto abb : boost::make_iterator_range(boost::vertices(calls))) {
				...
				if (cfg.abb_is_indirect<Graph>(abb)) {
					...
					llvm::CallBase* called_func = llvm::dyn_cast<llvm::CallBase>(
					    &reinterpret_cast<llvm::BasicBlock*>(cfg.entry_bb[abb])->front());
					...
					for (llvm::Function& candidate : mod) {
						if (candidate.getFunctionType() == called_type) {
							...
							auto other_abb = get_vertex<Graph>(
							    g, other_func, [&](typename boost::graph_traits<Graph>::edge_descriptor e) {
