...
namespace ara::step {
	...
	namespace {
		bool handle_tcb_ref_param(IRBuilder<> &Builder, Value* tcb_ref, Value* the_tcb, Logger &logger) {
			if (Constant* CI = dyn_cast<Constant>(tcb_ref)) {
				...
				logger.error() << "NOT IMLEMENTED: Need to generate runtime checking code for task_handle in xTaskCreate call" << std::endl;
			return false;
			}
			return true;
		}
	...
	bool ReplaceSyscallsCreate::replace_task_create_static(graph::Graph& graph, int where, char* tcb_name, char* stack_name) {
		...
		GlobalVariable *task_tcb = new GlobalVariable(module, dyn_cast<PointerType>(create_static_fn->getFunctionType()->getParamType(6))->getElementType(), false, GlobalValue::ExternalLinkage, nullptr, tcb_name, nullptr, GlobalVariable::NotThreadLocal, 0, false);
		GlobalVariable *task_stack = new GlobalVariable(module, dyn_cast<PointerType>(create_static_fn->getFunctionType()->getParamType(5))->getElementType(), false, GlobalValue::ExternalLinkage, nullptr, stack_name, nullptr, GlobalVariable::NotThreadLocal, 0, false);
