...
namespace step {
	...
	void do_graph_stuff(Graph& g, ara::graph::CFG& cfg, Logger& logger) {
		...
		for (auto abb : boost::make_iterator_range(boost::vertices(filter_by_abb(ara::graph::ABBType::syscall, g, cfg)))) {
	...
	std::string ValueAnalysis::get_description() const {
		return "Perform a value analysis for all system calls.";
	}
