...
class DumbArgumentWarning : public Warning {
  ...
  public:
	DumbArgumentWarning(int index, OS::shared_abb abb) : Warning(abb) { this->argument_index = index; }
	virtual std::string get_type() const override { return "DumpArgument"; }

	virtual std::string print_warning() const override {
		std::string stream = "";
		stream + "Argument at index " + std::to_string(argument_index) + " in abb " + warning_position->get_name() +
		    " could not dump" + "\n";
		return stream;
	};
};
...
class EdgeCreateWarning : public Warning {
  ...
  public:
	...
	virtual std::string print_warning() const override {
		std::string stream = "";
		stream + "Edge from " + warning_position->get_name() + " in vertex " + start_vertex->get_name() +
		    " could not created" + "\n";
		return stream;
	};
};
...
class TopologicalOrderWarning : public Warning {
  ...
  public:
	...
	virtual std::string print_warning() const override {
		std::string stream = "";
		stream + "Function " + function->get_name() + " could not topolicial sorted at abb " +
		    warning_position->get_name() + "\n";
		return stream;
	};
};
...
class FreeRTOSCreateInstanceWarning : public Warning {
  ...
  public:
	...
	virtual std::string print_warning() const override {
		std::string stream = "";
		stream + "In abb " + warning_position->get_name() + " FreeRTOS instance from class" + instance_class +
		    "could not created\n";
		return stream;
	};
};
...
class BufferWarning : public Warning {
  ...
  public:
	BufferWarning(OS::shared_buffer buffer, OS::shared_abb abb) : Warning(abb) { this->buffer = buffer; }
...
class QueueSetWarning : public Warning {
  ...
  public:
	QueueSetWarning(OS::shared_queueset queueset, graph::shared_vertex accessed_element, OS::shared_abb abb)
	    : Warning(abb) {
		this->queueset = queueset;
	...
	virtual std::string print_warning() const override {
		std::string stream = "";
		stream += "QueueSet " + queueset->get_name() + " element " + accessed_element->get_name() +
		          " is accessed directly without taking from queueset before" + "\n";
		return stream;
	};
};
...
class EventWarning : public Warning {
  ...
  public:
	EventWarning(OS::shared_event event, graph::shared_vertex waiting_element, unsigned long expected_bits,
	             unsigned long set_bits, OS::shared_abb abb)
	    : Warning(abb) {
		this->event = event;
...
class EventWarningWrongListSize : public Warning {
  ...
  public:
	EventWarningWrongListSize(OS::shared_event event, graph::shared_vertex waiting_element,
	                          unsigned long eventlist_size, unsigned long set_expected_bits, OS::shared_abb abb)
	    : Warning(abb) {
		this->event = event;
		this->waiting_element = waiting_element;
	...
	virtual std::string print_warning() const override {
		std::string stream = "";
		stream += "Vertex " + waiting_element->get_name() + "set/wait for bits in event " + event->get_name() +
		          "with to less size\n";
...
class EnableWarning : public Warning {
  ...
  public:
	...
	virtual std::string print_warning() const override {
		...
		stream += "Abstraction instance " + instance->get_name() +
		          " is used without enabling the abstraction class in rtos configuration" + "\n";
		return stream;
	};
...
class ISRSyscallWarning : public Warning {
  ...
  public:
	...
	virtual std::string print_warning() const override {
		std::string stream = "";
		stream += "ISR " + isr->get_name() + " use syscall " + abb->get_syscall_name() +
		          " without isr syscall api use (FromISR expected)" + "\n";
		return stream;
	};
};
...
class CriticalRegionWarning : public Warning {
  ...
  public:
	CriticalRegionWarning(graph::shared_vertex vertex, OS::shared_abb abb) : Warning(abb) { this->vertex = vertex; };

	virtual std::string get_type() const override { return "CriticalRegion"; }

	virtual std::string print_warning() const override {
		std::string stream = "";
		stream += "Critical region starting in abb  " + warning_position->get_name() + " with syscall " +
		          warning_position->get_syscall_name() + " does not certainly end in instance " + vertex->get_name() +
		          "\n";
		return stream;
	};
};
...
class SemaphoreUseWarning : public Warning {
  ...
  public:
	SemaphoreUseWarning(OS::shared_semaphore semaphore, graph::shared_vertex vertex, OS::shared_abb abb)
	    : Warning(abb) {
	...
	virtual std::string print_warning() const override {
		std::string stream = "";
		stream += "Semaphore" + semaphore->get_name() + "is taken in " + vertex->get_name() +
		          "but not given in other instance" + "\n";
		return stream;
	}
};
...
class MutexUseWarning : public Warning {
  ...
  public:
	...
	virtual std::string print_warning() const override {
		std::string stream = "";
		stream += "Resouce" + resouce->get_name() + "is taken in " + vertex->get_name() +
		          "but not given in same instance" + "\n";
		return stream;
	}
};
...
class PriorityInversionWarning : public Warning {
  ...
  public:
	PriorityInversionWarning(std::vector<OS::shared_task>* indirect_blocking_vertexes, OS::shared_task min_prio_vertex,
	                         OS::shared_task max_prio_vertex, OS::shared_resource resource, OS::shared_abb abb)
	    : Warning(abb) {
	...
	virtual std::string print_warning() const override {
		...
		stream += "Vertex min " + min_prio_vertex->get_name() + " with priority " +
		          std::to_string(min_prio_vertex->get_priority()) + "\n";
		stream += "Vertex max " + max_prio_vertex->get_name() + " with priority " +
		          std::to_string(max_prio_vertex->get_priority()) + "\n";

		for (auto indirect_blocking_vertex : indirect_blocking_vertexes) {
			stream += "Vertex " + indirect_blocking_vertex->get_name() + " with priority " +
			          std::to_string(indirect_blocking_vertex->get_priority()) + " can block vertex max indirect" +
			          "\n";
		}
		stream += "\n";
		return stream;
...
inline std::ostream& operator<<(std::ostream& stream, const Warning& warning) {
