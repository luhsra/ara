 ...
 */
bool validate_instructions_reachability(std::vector<llvm::Instruction*>* instruction_list,
                                        llvm::Instruction* instruction, llvm::DominatorTree* dominator_tree) {
	bool success = false;
 ...
 */
bool validate_instructions_dominance(std::vector<llvm::Instruction*>* instruction_list, llvm::Instruction* instruction,
 ...
 */
bool validate_one_instructions_dominance(std::vector<llvm::Instruction*>* instruction_list,
                                         llvm::Instruction* instruction, llvm::DominatorTree* dominator_tree) {
	bool success = false;
	for (auto* tmp_instruction : *instruction_list) {
		if (dominator_tree->dominates(tmp_instruction, instruction)) {
 ...
 */
void update_called_functions(graph::Graph& graph, OS::shared_function function,
                             std::vector<std::size_t>* already_visited, start_scheduler_relation state) {
	...
	for (auto& abb : function->get_atomic_basic_blocks()) {
		if (abb->get_call_type() == sys_call) {
			...
			if (start_scheduler == abb->get_syscall_type())
				instruction_vector.push_back(abb->get_syscall_instruction_reference());
		}
	}
...
/**
 * @brief set for each abb in the main function the information if the abb is before, uncertain oder behind the start
 * scheduler call
 * @param graph project data structure
...
start_scheduler_relation before_scheduler_instructions(graph::Graph& graph, OS::shared_function function,
			...
			for (auto& abb : function->get_atomic_basic_blocks()) {
				...
				for (auto predecessor : abb->get_ABB_predecessors()) {
					...
					if (predecessor->get_start_scheduler_relation() == after)
						after_flag = true;
					if (predecessor->get_start_scheduler_relation() == uncertain)
						uncertain_flag = true;
				}
				...
				if (tmp_state == uncertain) {
					...
					if (abb->get_entry_bb() != nullptr) {
						if (validate_instructions_reachability(&start_scheduler_func_calls,
						                                       &(abb->get_entry_bb()->front()), dominator_tree) ||
						    validate_instructions_reachability(&uncertain_start_scheduler_func_calls,
						                                       &(abb->get_entry_bb()->front()), dominator_tree))
							tmp_state = uncertain;
					}
				}
	...
	if (function->get_exit_abb() != nullptr)
		return function->get_exit_abb()->get_start_scheduler_relation();
	else
		return after;
}
 ...
 */
void SCC_topological_sort(llvm::BasicBlock* bb, std::map<std::string, std::string>* open_predecessors,
                          std::map<std::string, std::string>* already_visited,
                          std::list<llvm::BasicBlock*>* topological_order, llvm::BasicBlock* end) {
...
void sort_abbs(graph::Graph& graph, std::vector<shared_warning>* warning_list) {
	...
	for (auto& vertex : vertex_list) {
		...
		for (scc_iterator<llvm::Function*> I = scc_begin(function->get_llvm_reference()),
		                                   IE = scc_end(function->get_llvm_reference());
		     I != IE; ++I) {
			...
			for (std::vector<BasicBlock*>::const_iterator BBI = SCCBBs.begin(), BBIE = SCCBBs.end(); BBI != BBIE;
			     ++BBI) {
			...
			for (auto& abb : abb_list) {
				...
				if (flag) {
					...
					for (auto bb : *unsorted_bb_list) {
						...
						int tmp_counter = 0;
						bool tmp_flag = false;
						while ((counter + tmp_counter) < topological_order.size() &&
						       tmp_counter < sorted_bb_list.size()) {
		...
		if (abb_list.size() == sorted_abb_list.size()) {
			...
			for (auto abb : sorted_abb_list) {
				if (function->get_exit_abb()->get_seed() == abb->get_seed())
					flag = false;
			}
			// check if missing abb is the exit abb of the function (because the exit abb may contain no bb)
			if (flag && abb_list.size() > sorted_abb_list.size() && function->get_exit_abb() != nullptr &&
			    function->get_exit_abb()->get_seed() != sorted_abb_list.back()->get_seed()) {
...
bool validate_loop(OS::shared_abb abb, std::map<size_t, size_t>* already_visited) {
	...
	if (abb->get_call_type() == sys_call)
		instr = abb->get_syscall_instruction_reference();
	else if (abb->get_call_type() == func_call)
		instr = abb->get_call_instruction_reference();
	else {
...
/**
 * @brief stores all system informationm which are stored in global llvm values. This global value are generated with
 *defines from the original configuration rtos configuration files with the preprocessor
 * @param graph project data structure
 **/
...
namespace step {
	...
	std::vector<std::string> IntermediateAnalysisStep::get_dependencies() { return {"ABB_MergeStep"}; }
} // namespace step
// RAII
