...
namespace llvm {
	...
	unsigned int getNumSuccessors(const llvm::Instruction* instr) {
	...
	const BasicBlock* getSuccessor(unsigned int index, const llvm::Instruction* instr) {
		unsigned int count = 0;
		const BasicBlock* bb = instr->getParent();
		if (bb == nullptr)
			return nullptr;
...
#ifndef NDEBUG
		unsigned e = getNumSuccessors(Term);
#endif
		for (unsigned i = 0;; ++i) {
			assert(i != e && "Didn't find edge?");
			if (getSuccessor(i, Term) == Succ)
				return i;
		}
	}
	...
	bool isCriticalEdge(const Instruction* TI, unsigned SuccNum, bool AllowIdenticalEdges) {
		assert(TI->isTerminator() && "Must be a terminator to have successors!");
		assert(SuccNum < getNumSuccessors(TI) && "Illegal edge specification!");
		if (getNumSuccessors(TI) == 1)
			return false;

		const BasicBlock* Dest = getSuccessor(SuccNum, TI);
	...
	bool isPotentiallyReachable(const BasicBlock* A, const BasicBlock* B, const DominatorTree* DT, const LoopInfo* LI) {
		assert(A->getParent() == B->getParent() && "This analysis is function-local!");

		SmallVector<BasicBlock*, 32> Worklist;
		Worklist.push_back(const_cast<BasicBlock*>(A));

		return isPotentiallyReachableFromMany(Worklist, const_cast<BasicBlock*>(B), DT, LI);
	}

	bool isPotentiallyReachable(const Instruction* A, const Instruction* B, const DominatorTree* DT,
	                            const LoopInfo* LI) {
		assert(A->getParent()->getParent() == B->getParent()->getParent() && "This analysis is function-local!");
		...
		return isPotentiallyReachableFromMany(Worklist, const_cast<BasicBlock*>(B->getParent()), DT, LI);
	}
} // namespace llvm
