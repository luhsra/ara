...
bool is_in_calltree(shared_function definition_function, shared_function current_function) {
	...
	while (!queue.empty()) {
		...
		if (current_function->get_seed() == function->get_seed())
			return true;
 ...
 */
bool is_reachable(shared_function definition_function, shared_abb start, shared_abb end,
                  std::map<size_t, size_t>* already_visited) {
	...
	while (!queue.empty()) {
		...
		if (!visited) {
			...
			if (abb->get_call_type() == func_call) {
				...
				if (called_function != nullptr) {
					if (called_function->get_exit_abb() == nullptr)
						call_return = false;
					if (called_function != nullptr)
				...
				if (is_in_calltree(definition_function, calling_abb->get_parent_function())) {
					for (auto successor : calling_abb->get_ABB_successors()) {
						if (already_visited->find(successor->get_seed()) == already_visited->end()) {
							if (is_reachable(definition_function, successor, end, already_visited))
								return true;
						}
					}
				}
...
llvm::Instruction* get_instruction(shared_abb abb) {
	...
	if (abb->get_call_type() == sys_call)
		return abb->get_syscall_instruction_reference();
	else if (abb->get_call_type() == func_call)
		return abb->get_call_instruction_reference();
 ...
 */
bool is_dominated(shared_function function, shared_abb start, shared_abb end,
                  std::map<size_t, size_t>* already_visited) {
		...
		while (!queue.empty()) {
			...
			if (!visited) {
				...
				if (abb->get_call_type() == func_call) {
					...
					// if func call abb is dominated by start abb and called function is not already visited analyse
					// dominance in called function recursive
					if (dominator_tree->dominates(start_instruction, end_instruction)) {
						auto called_function = abb->get_called_function();
						if (called_function->get_exit_abb() == nullptr)
							call_return = false;
						if (already_visited->find(called_function->get_entry_abb()->get_seed()) ==
						    already_visited->end()) {
 ...
 */
bool detect_critical_region_in_function(shared_function definition_function, shared_function function,
                                        shared_abb entry_abb, std::map<size_t, size_t>* already_visited,
                                        bool recursion_upside, bool& exit_definition_function) {
	...
	if (function->get_exit_abb() != nullptr)
		exit_abb = function->get_exit_abb();
	if (definition_function->get_exit_abb() != nullptr)
		definition_exit_abb = definition_function->get_exit_abb();
	...
	while (!queue.empty()) {
		...
		if (!visited) {
			...
			if (exit_abb != nullptr && exit_abb->get_seed() == abb->get_seed())
				reaches_exit_abb = true;
			if (definition_exit_abb != nullptr && definition_exit_abb->get_seed() == abb->get_seed())
				exit_definition_function = true;
			...
			if (abb->get_call_type() == func_call) {
				...
				if (called_function != nullptr)
					entry_abb = called_function->get_entry_abb();
				if (!detect_critical_region_in_function(definition_function, called_function, entry_abb,
				                                        already_visited, false, exit_definition_function))
					analyse_successors = false;
			}
				...
				if (is_in_calltree(definition_function, calling_abb->get_parent_function())) {
					for (auto successor : calling_abb->get_ABB_successors()) {
						...
						if (already_visited->find(successor->get_seed()) == already_visited->end()) {
							...
							// std::cerr <<"exit abb upwards: " <<  exit_abb->get_name() << " " << reaches_exit_abb <<
							// std::endl;
							detect_critical_region_in_function(definition_function, calling_abb->get_parent_function(),
							                                   successor, already_visited, true,
							                                   exit_definition_function);
						}
					}
				}
			}
 ...
 */
bool detect_critical_regions(shared_function definition_function, std::list<graph::shared_edge>* start_calls,
                             std::list<graph::shared_edge>* end_calls) {
	...
	for (auto start_call : *start_calls) {
		...
		for (auto end_call : *end_calls) {
			already_visited.insert(
			    std::make_pair(end_call->get_abb_reference()->get_seed(), end_call->get_abb_reference()->get_seed()));
		}
		...
		detect_critical_region_in_function(definition_function, start_abb->get_parent_function(), start_abb,
		                                   &already_visited, true, exit_definition_function);
	}
...
void verify_mutexes(graph::Graph& graph, std::vector<shared_warning>* warning_list) {
	...
	for (auto& vertex : graph.get_type_vertices<OS::Mutex>()) {
		...
		for (auto ingoing : ingoing_edges) {
			...
			if (ingoing->get_abb_reference()->get_syscall_type() == commit)
				mutex_gives.emplace_back(ingoing);
			// std::cerr  << "in " <<  ingoing->get_name() << std::endl;
		}

		auto outgoing_edges = resource->get_outgoing_edges();
		for (auto outgoing : outgoing_edges) {
			if (outgoing->get_abb_reference()->get_syscall_type() == take)
				mutex_takes.emplace_back(outgoing);
		...
		if (resource->get_resource_type() == binary_mutex || resource->get_resource_type() == recursive_mutex) {
			...
			for (auto take : mutex_takes) {
				...
				for (auto give : mutex_gives) {
					// std::cerr <<  "start " << give->get_start_vertex()->get_name() <<  "end" <<
					// take->get_target_vertex()->get_name()  << std::endl; check if take and give are done by same task
					if (give->get_start_vertex()->get_seed() == take->get_target_vertex()->get_seed()) {
						...
						if (is_reachable(definition_function, take->get_abb_reference(), give->get_abb_reference(),
						                 nullptr))
							mutex_flag = true;
					}
				}
				if (!mutex_flag) {
					auto warning = std::make_shared<MutexUseWarning>(resource, take->get_start_vertex(),
					                                                 take->get_abb_reference());
...
void verify_semaphores(graph::Graph& graph, std::vector<shared_warning>* warning_list) {
	...
	for (auto& vertex : graph.get_type_vertices<OS::Semaphore>()) {
		...
		for (auto ingoing : ingoing_edges) {
			...
			if (ingoing->get_abb_reference()->get_syscall_type() == commit)
				semaphore_gives.emplace_back(ingoing);
			// std::cerr  << "in " <<  ingoing->get_name() << std::endl;
		}
		...
		for (auto outgoing : outgoing_edges) {
			if (outgoing->get_abb_reference()->get_syscall_type() == take)
				semaphore_takes.emplace_back(outgoing);
			// std::cerr <<  "out " << outgoing->get_name() << std::endl;
		}

		// check if the semaphore is from right type
		if (semaphore->get_semaphore_type() == binary_semaphore ||
		    semaphore->get_semaphore_type() == counting_semaphore) {
			...
			for (auto take : semaphore_takes) {
				...
				if (!semaphore_flag) {
					...
					auto warning = std::make_shared<SemaphoreUseWarning>(semaphore, take->get_start_vertex(),
					                                                     take->get_abb_reference());
					warning_list->emplace_back(warning);
				}
			}
			...
			for (auto give : semaphore_gives) {
				...
				if (!semaphore_flag) {
					// error no take and give from different abstraction instances-> store warning
					auto warning = std::make_shared<SemaphoreUseWarning>(semaphore, give->get_start_vertex(),
					                                                     give->get_abb_reference());
					warning_list->emplace_back(warning);
				}
			}
		}
...
void verify_freertos_events(graph::Graph& graph, std::vector<shared_warning>* warning_list) {
	...
	for (auto& vertex : graph.get_type_vertices<OS::Event>()) {
		...
		for (auto ingoing : ingoing_edges) {
			...
			if (type == commit || type == synchronize)
				set_calls.emplace_back(ingoing);
			// std::cerr  << "in " <<  ingoing->get_name() << std::endl;
		}
		...
		for (auto outgoing : outgoing_edges) {
			...
			if (type == wait || type == synchronize)
				wait_calls.emplace_back(outgoing);
			// std::cerr <<  "out " << outgoing->get_name() << std::endl;
		}
		...
		for (auto wait_call : wait_calls) {
			...
			if (type == synchronize) {
				...
				// std::cerr << "wait bits " << wait_bits << ", clear on exit " << clear_on_exit << ", wait_for_all_bits
				// " << wait_for_all_bits << std::endl;
			}
			...
			for (auto set_call : set_calls) {
				...
				if (set_instance == wait_instance) {
					...
					if (is_reachable(definition_function, wait_call->get_abb_reference(), set_call->get_abb_reference(),
					                 nullptr))
						continue;
				}
			...
			if (rtos->support_16_bit_ticks == true) {
				...
				if (set_bits >= 256 || wait_bits >= 256) {
					...
					if (set_bits > wait_bits)
						...
						bits = wait_bits;
					auto warning = std::make_shared<EventWarningWrongListSize>(event, wait_call->get_target_vertex(),
					                                                           bits, 8, nullptr);
					warning_list->emplace_back(warning);
				}
			...
			if (wait_for_all_bits) {
				if (!((set_bits & wait_bits) == wait_bits)) {
					...
					auto warning = std::make_shared<EventWarning>(event, wait_call->get_target_vertex(), wait_bits,
					                                              set_bits, wait_call->get_abb_reference());
					warning_list->emplace_back(warning);
				}
			} else {
				// std::cerr << "notwaitforall, set bits " << set_bits << std::endl;
				// std::cerr << "wait bits " << wait_bits << ", clear on exit " << clear_on_exit << ", wait_for_all_bits
				// " << wait_for_all_bits << std::endl; std::cerr << (set_bits & wait_bits) << std::endl;
				if (!((set_bits & wait_bits))) {
					auto warning = std::make_shared<EventWarning>(event, wait_call->get_target_vertex(), wait_bits,
					                                              set_bits, wait_call->get_abb_reference());
					warning_list->emplace_back(warning);
...
void verify_priority_inversion(graph::Graph& graph, std::vector<shared_warning>* warning_list) {
	...
	// inversion check if the resource is accessed by other instances (task,isr)  and their is another instance with a
	// lower priority that doesnt access the resource
	...
	for (auto& vertex : resource_list) {
		...
		for (auto outgoing_edge : resource->get_outgoing_edges()) {
			...
			if (typeid(OS::Task).hash_code() == accessed_vertex->get_type()) {
				...
				if (outgoing_edge->get_abb_reference()->get_syscall_type() != take || !task->has_constant_priority())
					continue;
				...
				for (auto tmp_outgoing_edge : resource->get_outgoing_edges()) {
					...
					if (typeid(OS::Task).hash_code() == tmp_accessed_vertex->get_type()) {
						...
						if (tmp_outgoing_edge->get_abb_reference()->get_syscall_type() != take ||
						    !tmp_task->has_constant_priority())
							continue;

						if (tmp_task->get_seed() == task->get_seed())
							continue;
				...
				if (resource_task_list.size() >= 2 && max_priority != min_priority) {
					// std::cerr << "possible bounded piority inversion: tasks of different priorities access resource "
					// << resource->get_name() << std::endl; std::cerr << "min priority " << min_priority << " max
					// priority " << max_priority << std::endl;
					std::vector<shared_task> unbouded_task_list;
					// check if there exists another task which doesnt access the resource and have a lower priority
					// than the max priority
					for (auto task_vertex : task_list) {
						...
						for (auto resource_task : resource_task_list) {
							if (task->get_seed() == resource_task->get_seed())
								flag = true;
						}
						if (flag == false) {
					...
					if (unbouded_task_list.size() > 0) {
						...
						auto warning = std::make_shared<PriorityInversionWarning>(
						    &unbouded_task_list, min_priority_task, max_priority_task, resource, abb);
						warning_list->emplace_back(warning);
					}
				}
			}
 ...
 */
bool find_cycle(graph::shared_vertex vertex, shared_abb abb, std::list<size_t>* visited, graph::Graph& graph, int depth,
                size_t start) {
	...
	for (auto edge : edges) {
		...
		if (edge->get_abb_reference()->get_syscall_type() != take)
			continue;
		...
		if (resource_perspective) {
			...
			if (target_vertex->get_type() != typeid(OS::Mutex).hash_code())
				continue;
			if (!is_reachable(definition_function, abb, edge->get_abb_reference(), nullptr))
				continue;
		}

		// recursion
		if (find_cycle(target_vertex, edge->get_abb_reference(), visited, graph, depth + 1, start))
			return true;
	}
...
/**
 * @brief //check if all interactions from type start (e.g disable) with the RTOS can be removed from interaction from
 *type end(e.genable) and determine critical abb regions (region between start and end)
 * @param graph project data structure
 ...
 */
void verify_specific_critical_region(graph::Graph& graph, graph::shared_vertex vertex,
                                     syscall_definition_type start_type, syscall_definition_type end_type,
                                     std::vector<shared_warning>* warning_list, size_t target, bool same_direction) {
	...
	if (same_direction) {
		for (auto outgoing_edge : vertex->get_outgoing_edges()) {
			...
			if (outgoing_edge->get_target_vertex()->get_seed() != target)
				continue;
			// check if the interaction disables functionalities
			if (outgoing_edge->get_abb_reference()->get_syscall_type() == start_type)
				start_interactions.emplace_back(outgoing_edge);
			else if (outgoing_edge->get_abb_reference()->get_syscall_type() == end_type)
		...
		for (auto ingoing_edge : vertex->get_ingoing_edges()) {
			// check if the interaction addresses the OS
			if (ingoing_edge->get_start_vertex()->get_seed() != target)
				continue;
			// check if the interaction disables functionalities
	...
	for (auto start_interaction : start_interactions) {
		...
		if (start_interactions.size() > 0 && end_interactions.size() == 0)
			success = false;
		else {
			...
			for (auto end_interaction : end_interactions) {
				...
				if (is_reachable(definition_function, start_interaction->get_abb_reference(),
				                 end_interaction->get_abb_reference(), nullptr))
					flag = true;
			}
			// detect critical region between start and end
			if (!flag)
				success = false;
			if (detect_critical_regions(definition_function, &start_interactions, &end_interactions))
				success = false;
		}
		if (success == false) {
...
/*
/**
* @brief //check if all interactions from type start (e.g disable) with the RTOS were removed certainly from interaction
from type end(e.genable) *and determine critical abb regions (region between start and end)
* @param graph project data structure
*/
void verify_critical_region(graph::Graph& graph, std::vector<shared_warning>* warning_list) {
	...
	for (auto& vertex : graph.get_type_vertices<OS::Task>()) {
		...
		// check if all interactions from type start (e.g disable) with the RTOS were removed certainly from interaction
		// from type end(e.genable)
		...
		verify_specific_critical_region(graph, task, enter_critical, exit_critical, warning_list, rtos->get_seed(),
		                                true);
		...
		for (auto edge : vertex->get_outgoing_edges()) {
			...
			if (edge->get_target_vertex()->get_type() == typeid(OS::Mutex).hash_code()) {
				...
				verify_specific_critical_region(graph, task, take, commit, warning_list,
				                                edge->get_target_vertex()->get_seed(), false);
			}
		}
	}
}
	...
	for (auto& vertex : graph.get_type_vertices<OS::QueueSet>()) {
		...
		for (auto queueset_element : queueset_elements) {
			...
			for (auto ingoing_edge : ingoing_edges) {
				...
				if (ingoing_edge->get_abb_reference()->get_syscall_type() == take ||
				    ingoing_edge->get_abb_reference()->get_syscall_type() == receive) {
					...
					for (auto queueset_ingoing_edge : queueset->get_ingoing_edges()) {
						if (queueset_ingoing_edge->get_abb_reference()->get_syscall_type() == take) {
							...
							auto definition_function =
							    get_definition_function(queueset_ingoing_edge->get_start_vertex());

							// the queueset element get element syscall is reachable from the queueset get element
							// syscall
							if (is_reachable(definition_function, queueset_ingoing_edge->get_abb_reference(),
							                 ingoing_edge->get_abb_reference(), nullptr)) {
								success = true;
								break;
							}
						}
					...
					if (!success) {
						...
						auto warning = std::make_shared<QueueSetWarning>(queueset, queueset_element,
						                                                 ingoing_edge->get_abb_reference());
						warning_list->emplace_back(warning);
					}
				}
			}
...
void verify_buffer_access(graph::Graph& graph, std::vector<shared_warning>* warning_list) {
	...
	for (auto& vertex : graph.get_type_vertices<OS::Buffer>()) {
		...
		for (auto outgoing_edge : buffer->get_outgoing_edges()) {
			if (outgoing_edge->get_abb_reference()->get_syscall_type() == commit ||
			    outgoing_edge->get_abb_reference()->get_syscall_type() == receive) {
		...
		for (auto ingoing_edge : buffer->get_ingoing_edges()) {
			if (ingoing_edge->get_abb_reference()->get_syscall_type() == commit ||
			    ingoing_edge->get_abb_reference()->get_syscall_type() == receive) {
...
namespace step {
	...
	/**
	 * @brief the run method of the ValidationStep pass. This pass detects all interactions of the instances via the
	 * RTOS.
	 * @param graph project data structure
	 */
