...
def check_instances_full_initialized(app_name, modified_app, generated_os, elf):
    ...
    if not re.search('InitializedStack_t<[0-9]+> txxx37_[0-9]_static_stack', generated_os):
...
def check_instances_full_static(app_name, modified_app, generated_os, elf):
    assert 'InitializedStack_t<10+> txxx40_5_static_stack' not in generated_os
    assert 'extern "C" StaticTask_t txxx40_5_tcb;' in generated_os
