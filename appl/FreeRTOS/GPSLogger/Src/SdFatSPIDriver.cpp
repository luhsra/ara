...
void SdFatSPIDriver::begin(uint8_t chipSelectPin)
{
	...
	// Init pins
	uint32_t MOSI = LL_GPIO_PIN_15;
	
	LL_GPIO_SetPinMode(GPIOB, MOSI, LL_GPIO_MODE_ALTERNATE);			// MOSI: AF PP
	LL_GPIO_SetPinOutputType(GPIOB, MOSI, LL_GPIO_OUTPUT_PUSHPULL);
	LL_GPIO_SetPinSpeed(GPIOB, MOSI, LL_GPIO_SPEED_FREQ_HIGH);

	uint32_t MISO = LL_GPIO_PIN_14;
	LL_GPIO_SetPinMode(GPIOB, MISO, LL_GPIO_MODE_INPUT); // MISO: Input Floating

	uint32_t SCK = LL_GPIO_PIN_13;
	LL_GPIO_SetPinMode(GPIOB, SCK, LL_GPIO_MODE_ALTERNATE);			// SCK: AF PP
	LL_GPIO_SetPinOutputType(GPIOB, SCK, LL_GPIO_OUTPUT_PUSHPULL);
	LL_GPIO_SetPinSpeed(GPIOB, SCK, LL_GPIO_SPEED_FREQ_HIGH);

	uint32_t CS = LL_GPIO_PIN_12;
	LL_GPIO_SetPinMode(GPIOB, CS, LL_GPIO_MODE_OUTPUT);				// CS
	LL_GPIO_SetPinOutputType(GPIOB, CS, LL_GPIO_OUTPUT_PUSHPULL);
	LL_GPIO_SetPinSpeed(GPIOB, CS, LL_GPIO_SPEED_FREQ_MEDIUM);
