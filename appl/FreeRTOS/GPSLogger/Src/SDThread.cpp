...
void ackRawGPSData(uint8_t len)
{
	...
	//	usbDebugWrite("Sending GPS message %d : %s -- %d\r\n",rawGPSDataBuf.msgIdx,rawGPSDataBuf.rawData.rawDataBuf, rawGPSDataBuf.rawData.len);
...
bool initSDCard()
{
	...
	//	if(!rawDataFile.open(&SD, "RAW_GPS.TXT", O_RDWR | O_CREAT | O_AT_END | O_SYNC)) {
	//usbDebugWrite("failed to open rawDataFie\r\n");
	//	}
...
{
    //serialDebugWrite("runSDMessageLoop\r\n"); 
	...
	while(true)
	{
		...
		if(xQueueReceive(sdQueue, &msg, 50)) // TO_not_DO: fix hardcode
		{
			...
			case RAW_GPS_DATA:
			  //		saveRawData(msg);
			  usbDebugWrite("Receive   GPS Data: %s\r\n",msg.rawData.rawDataBuf);
				break;
			default:
				break;
			}
