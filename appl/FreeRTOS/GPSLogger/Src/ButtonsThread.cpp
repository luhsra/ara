...
const uint32_t OK_BUTTON_PIN = LL_GPIO_PIN_12;
...
void initButtons()
{
	...
	LL_GPIO_SetPinPull(GPIOC, SEL_BUTTON_PIN, LL_GPIO_PULL_UP);
	LL_GPIO_SetPinMode(GPIOC, OK_BUTTON_PIN, LL_GPIO_MODE_INPUT);
	LL_GPIO_SetPinPull(GPIOC, OK_BUTTON_PIN, LL_GPIO_PULL_UP);
...
void vButtonsThread(void *pvParameters)
{
	for (;;)
	{
		...
		if (btn != NO_BUTTON)
		{
		  //serialDebugWrite("Button Press:%d\n\r", btn);
